...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    fprintf(stderr, "使用： %s <nproc> <total[ms]> <resolution[ms]>\n", argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  if (nproc < 1) {
    fprintf(stderr, "<nproc>(%d) は >= 1であるべき\n", nproc);
    exit(EXIT_FAILURE);
  }

  if (total < 1) {
    fprintf(stderr, "<total>(%d)は >= 1であるべき\n", total);
    exit(EXIT_FAILURE);
  }

  if (resol < 1) {
    fprintf(stderr, "<resol>(%d)は >= 1であるべき\n", resol);
    exit(EXIT_FAILURE);
  }

  if (total % resol) {
    fprintf(stderr, "<total>(%d)は<resolution>(%d)で割り切れるべき\n", total, resol);
    exit(EXIT_FAILURE);
  }
